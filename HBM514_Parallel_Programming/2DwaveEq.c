...
// https://github.com/sametd/2d_wave_mpi/blob/main/waveEq.c
...
ncFileProps create_ncFile() {
    ...
    for (int i = 0; i < grid_size; ++i) {
        grid_x_points[i] = (-(grid_size / 2) + i) * 6e-3;
        grid_y_points[i] = (-(grid_size / 2) + i) * 6e-3;
    }
...
void apply_initial_cond(double* vec) {
    for (int i = 0; i < grid_size; i++) {
        for (int j = 0; j < grid_size; j++) {
            vec[i * grid_size + j] = i * 6e-3 * ((3.14159)-i * (6e-3)) * j * (6e-3) * ((3.14159)-j * (6e-3));
        }
    }
}
